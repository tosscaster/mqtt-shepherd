...
Mqdb.prototype.replace = function (cId, path, value, callback) {
  ...
  if (path === 'clientId') {
    ...
    this.findByClientId(cId).then(function (so) {
      if (!so) { throw new Error('No such object ' + cId + ' for property replacing.') } else if (!_.has(so, path)) { throw new new Error('No such property ' + path + ' to replace.')() } else { return Q.ninvoke(self.db, 'update', { clientId: cId }, { $set: objToReplace }, {}) }
...
Mqdb.prototype.modify = function (cId, path, snippet, callback) {
  ...
  var foundSo
  var deferred = Q.defer()
  var pLength = path.length + 1
  var diffSnippet = {}
  var invalidPath
  var objToUpdate = mutils.buildPathValuePairs(path, snippet)
  ...
  this.findByClientId(cId).then(function (so) {
    if (!so) {
      ...
      if (invalidPath.length !== 0) { throw new Error('No such property ' + invalidPath[0] + ' to modify.') } else { return Q.ninvoke(self.db, 'update', { clientId: cId }, { $set: objToUpdate }, { multi: true }) }
    }
  }).then(function () {
    ...
    _.forEach(objToUpdate, function (val, checkPath) {
      ...
      var newVal = _.get(newSo, checkPath)
      var oldVal = _.get(foundSo, checkPath)
