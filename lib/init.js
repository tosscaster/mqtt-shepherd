...
init.setupShepherd = function (shepherd, callback) {
  ...
  init._loadNodesFromDb(shepherd).then(() => {
    ...
    if (!shepherd.mBroker) {
      ...
      shepherd.mBroker = require("net").createServer(aedes.handle)
      debug('Broker is up.')
    } else {
      setTimeout(() => {
        debug('Broker is up.')
    ...
    broker.listen(shepherd.brokerSettings.port, function() {
        debug('server started and listening on port ' + shepherd.brokerSettings.port);
        initProcedure()
      })
  }).fail((err) => {
    deferred.reject(err)
...
init._setupAuthPolicy = function (shepherd) {
  ...
  broker.authorizePublish = function(client, packet, cb) {
    ...
    if (client.id === shepherdId) { // shepherd can always publish
      ...
      if (shepherd.authPolicy && _.isFunction(shepherd.authPolicy.authorizePublish)) shepherd.authPolicy.authorizePublish(client, packet.topic, packet.payload, cb)
      else cb(null, true)
    }
  }
  ...
  broker.authorizeSubscribe = function(client, sub, cb) {
    ...
    if (client.id === shepherdId) { // shepherd can always subscribe
      ...
      if (shepherd.authPolicy && _.isFunction(shepherd.authPolicy.authorizeSubscribe)) shepherd.authPolicy.authorizeSubscribe(client, sub.topic, cb)
      else cb(null, true)
    }
  }
  ...
  broker.authorizeForward = function(client, packet) {
    if (client.id === shepherdId) {
      ...
      shepherd.authPolicy.authorizeForward(client, packet);
    } else {
      return packet;
    }
  }
...
init._attachBrokerEventListeners = function (shepherd) {
  ...
  broker.on("subscribe", (topic, client) => {
