...
init.setupShepherd = function (shepherd, callback) {
  ...
  init._loadNodesFromDb(shepherd).then(() => {
    ...
    if (!shepherd.mBroker) {
      ...
      shepherd.mServer = require('net').createServer(aedes.handle);
      shepherd.mServer.listen(shepherd.brokerSettings.port, () => {
...
init._attachBrokerEventListeners = function (shepherd) {
  ...
  broker.on('client', client => {
    debug(`broker:clientConnected <- ${client.id}`);
  ...
  broker.on('clientDisconnect', client => {
    debug(`broker:clientDisconnect <- ${client.id}`);
    const qnode = shepherd._nodebox[client.id]
  ...
  broker.on('publish', (packet, client) => {
    //if (client) {
    //  debug("broker:publish ", client.id);
    //}
  ...
  broker.on('subscribe', (subscriptions, client) => {
    //debug(`broker:subscribe <- ${client.id}`);
  ...
  broker.on('unsubscribe', (unsubscriptions, client) => {
    //debug(`broker:unsubscribe <- ${client.id}`);
...
init._dispatchMessage = function (shepherd, intf, cId, topic, message) {
  ...
  console.log("<-- msg(intf/cid: transid)", topic, parsedMsg.transId);
