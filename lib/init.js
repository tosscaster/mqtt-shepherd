...
init.setupShepherd = function (shepherd, callback) {
  ...
  //]
  const mBrokerEvents = [ // Event names for removing broker listeners
    'client', 'clientDisconnect', 'publish', 'subscribe ', 'unsubscribe'
  ]
...
init._attachBrokerEventListeners = function (shepherd) {
  ...
  broker.on('subscribe', (subscriptions, client) => {
    debug(`subscribe <- ${subscriptions}`)
    if (client.id !== shepherdId) shepherd.emit('priphSubscribed', subscriptions.topic, client)
  })

  //broker.on('unsubscribed', (topic, client) => {
  broker.on('unsubscribe', (unsubscriptions, client) => {
    debug(`unsubscribe <- ${unsubscriptions}`)
    if (client.id !== shepherdId) shepherd.emit('priphUnsubscribed', unsubscriptions.topic, client)
  })
  */
