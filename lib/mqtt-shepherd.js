...
var network = IS_TESTING ? require('./components/network_mock') : require('network')
var debug = require('debug')('mqtt-shepherd')
var logReq = require('debug')('mqtt-shepherd:request')
var init = require('./init')
var Mqdb = require('./components/mqdb')
...
function MShepherd (name, settings) {
  ...
  var __eventsCount = this._eventsCount

  var __maxListeners = this._maxListeners
  ...
  if (!_.isNil(name) && !_.isString(name)) { throw new TypeError('name should be a string if gieven.') } else if (!_.isPlainObject(settings)) { throw new TypeError('settings should be an object if gieven.') }
      ...
      if (self._mqdb) {
        self._mqdb.db.remove({}, { multi: true }, function (err, num) {
          ...
          self._mqdb.db.loadDatabase(function (err) {
            if (!err) { debug('Database cleared.') } else { debug(err) }
          })
        })
      } else {
...
MShepherd.prototype.findByMac = function (macAddr) {
  if (!_.isString(macAddr)) { throw new TypeError('macAddr should be a string.') } else { macAddr = macAddr.toLowerCase() }
...
MShepherd.prototype._responseSender = function (intf, clientId, rspObj, callback) {
  ...
  var topic = intf + '/response/' + clientId

  var msg = JSON.stringify(rspObj) // rspObj won't be changed if it is a string
...
MShepherd.prototype._requestSender = function (cmdId, clientId, reqObj, waitTime, callback) {
  ...
  var cmdIdString = mutils.cmdKey(cmdId)
  ...
  if (arguments.length < 2) { preError = new Error('Bad arguments.') } else if (!this._enabled) { preError = new Error('Shepherd is not ready, cannot send request.') } else if (!qnode) { preError = new Error('No such node, clientId: ' + clientId) } else if (qnode.getStatus() === 'offline') { preError = new Error('Client offline, clientId: ' + qnode.clientId) }
  ...
  if (qnode.getStatus() === 'sleep' && !quickping) {
    qnode.quickPingReq().then(function () {
      ...
      if (couldWait) { doSending() } else { deferred.reject('Client is sleeping, clientId: ' + qnode.clientId) }
    }).done()
  } else {
    doSending()
  }
...
MShepherd.prototype.list = function (cIds) {
  ...
  if (_.isString(cIds)) { cIds = [ cIds ] } else if (!_.isUndefined(cIds) && !_.isArray(cIds)) { throw new TypeError('cIds should be a string or an array of strings if given.') } else if (!cIds) { cIds = _.keys(this._nodebox) } // list all

  foundNodes = _.map(cIds, function (cid) {
    var rec

    var found = self.find(cid)
...
MShepherd.prototype.devListMaintain = function (qnode, callback) {
  ...
  if (_.isArray(qnode)) { nodes = qnode } else if (qnode) { nodes = [ qnode ] } else { nodes = this._nodebox }
